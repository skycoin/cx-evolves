...
func (game *Game) RandomPlayer(gameMove *GameMove) AgentInput {
	var input int
	var options []int
	...
	if agentInput.BlindRunner {
		...
		if game.maze.IsWallOpen(gameMove.AgentPosition, Down) && checkPrev(CheckPrev, gameMove.PrevInput, Up) {
		...
		if game.maze.IsWallOpen(gameMove.AgentPosition, Left) && checkPrev(CheckPrev, gameMove.PrevInput, Right) {
		...
		if game.maze.IsWallOpen(gameMove.AgentPosition, Right) && checkPrev(CheckPrev, gameMove.PrevInput, Left) {
	...
	for input == 0 {
		...
		if len(options) == 0 {
			...
			input = game.RandomPlayer(gameMove).Move
		}
	}
